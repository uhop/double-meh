...
**Verdict:** `double-meh` is a superset of the heya libraries on the *conceptual* surface (pipeline,
...
| `retries`, `nextDelay`, `initDelay` | same (on `io.retry`) | ✅ |
| `bundle`, `wait`, `bust` | — | 🔲 (bundle/bust not yet; `wait` unwired) |
| `continueRetries` | — | 🔲 |
| `timeout` | — | 🔲 (`options.timeout` → `AbortController`) |
...
| **track** (`wait:true` register interest) | dedup ✅; `wait` unwired 🔲 | 🔲 |
| **bust** (`bust:true` / custom key) | — | 🔲 (or manual `query`) |
| **retry** (`retries:0`, `continueRetries: r => r.status>=500`, delays) | `retries`/delays ✅; `continueRetries` 🔲 | ✅ / 🔲 |
| **jsonp** | — | ❌ dropped |
| **load** | — | ❌ dropped |
...
`options.timeout`; persistent cache backends + `Vary` keying; **fetch-init passthrough**
(`credentials`/`mode`/`redirect`/`referrer`/`integrity` — the one genuine coverage gap worth
prioritizing); `wait` (type-declared, unwired).
...
redesign; the only *non-deliberate* gaps are the queued items above, of which **fetch-init
passthrough** is the one that could surprise a migrating heya user.
