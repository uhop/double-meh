...
- **Pluggable types & envelopes** _(settled 2026-07-09)_. The plugin surface is the existing seams,
  composed — no new machinery: `io.mimeTypes` (the `as` alias), `registerData` (request wrap, full
  `(data, headers, options)` context), `registerMime` (response unwrap; `decode(response, options)` —
  options added for per-request context; runs on error bodies, feeding `BadStatus.problem`), response
  inspectors (envelope rework on _shared_ MIME types that must not be hijacked globally), and
  ...
  **SOAP is a separate project** consuming this surface; Eugene's JSON↔XML reflection gist
  ...
  reuse the approach, not the code verbatim, and compare results against it (it is working code).
...
- **Upload progress** — `fetch` has none natively. _(built 2026-07-09:)_ `onUploadProgress` meters
  bytes handed to the transport, inserted at the innermost dispatch (post-compression, real wire
  only — mock/cache hits report nothing). Stream bodies get a technical pass-through
  `TransformStream` counting per chunk (`lengthComputable: false`; a locked stream is left alone —
  a spent retry attempt surfaces the transport's own error). **Buffered bodies stay buffered** —
  converting to a stream would silently break h1 and non-Chromium uploads — so they get one
  completion event (`loaded === total`, UTF-8 byte size / `Blob.size`) when the response arrives;
  `FormData`/`URLSearchParams` report nothing (the platform owns their final encoding). The shared
  `Progress` shape ({loaded, total, lengthComputable}) serves both directions.
...
| Tier                           | Items                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **v1 (this repo)**             | fetch transport; verbs + `io.full` + the envelope; JSON fast path; data/MIME processors; request ergonomics (conditional, idempotency, query builders, `as`, `accept`); safety-aware retry; `problem+json`; inspectors; events; cache (browser: Cache API + IndexedDB + Storage; CLI: in-memory default + filesystem; app-governed TTL, 304 revalidation, invalidation glue); track; mock; bust; `url`; download + upload progress; pagination iteration; pluggable types/envelopes (plugin surface); bundle client; compression encoders (CLI); JSONL/json-seq record iteration (`io.records`, decode side; processing/encode delegated to `stream-chain`); `io.stream` duplexes; code-forward (early-init + request adoption); canonical `makeKey`; SW invalidation message-contract + cross-tab (BroadcastChannel); SSE (`io.sse`, rides fetch+streaming) |
| **Spike to size, then decide** | — (SSE resolved → v1)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| **Research / later**           | offline + background-sync writes (SW)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| **Separate repos**             | bundle _server_ example; reference Service Worker; SOAP plugin (on the pluggable-envelope surface)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
