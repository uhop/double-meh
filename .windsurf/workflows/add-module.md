---
description: Checklist for adding a new utility or module to double-meh
---
...
1. Create `src/utils/foo.js` with the implementation.
   - ESM (`export default` / `export`). Use `import` for imports.
   - No runtime dependencies — only import from `node:` or other `src/` modules.
...
3. Create `tests/test-foo.mjs` with automated tests (tape-six, ESM):
   - Import the utility and test its API.
   - Test normal operation, edge cases, and integration.
